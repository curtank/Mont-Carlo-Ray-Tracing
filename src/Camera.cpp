...
Camera::Camera() {
	fov = 37.0f;
	orign = glm::vec3(0.0f, 0.0f, 4.0f);
	nvector = glm::vec3(0.0f, 0.0f, -1.0f);
	theta = 0.0f;
	angle = tan(M_PI * 0.5 * fov / 180.0);
	glm::mat4 orignmat = glm::mat4();
	transformMat = glm::rotate(orignmat, theta, nvector);
}
Camera::Camera(float fov,glm::vec3 orignposition,glm::vec3 nvector) {
	this->fov = fov;
	angle = tan(M_PI * 0.5 * fov / 180.0);
}
Camera::~Camera() {
...
Ray Camera::GetRandomRayDirection(int x, int y, Image* img) {
  ...
  float v = (float)rand()/RAND_MAX;
	...
	Ray ray;
	ray.direction =transformMat *glm::normalize(glm::vec3(xx, yy, -1.0f)); // Look in negative z-direction
	ray.origin = glm::vec3(0.0f, 0.0f, 4.0f); 
